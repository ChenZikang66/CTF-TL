...
class ProposalBranch(nn.Module):
    def __init__(self, in_channels, proposal_channels):
        ...
        self.feature_transformer = TransformerBlock(
            dim=in_channels,
            num_heads=8,
            mlp_ratio=4.,
            dropout=0.1
        ...
        self.frame_transformer = TransformerBlock(
            dim=in_channels,
            num_heads=8,
            mlp_ratio=4.,
            dropout=0.1
    ...
    def forward(self, feature, frame_level_feature, segments, frame_segments):
        ...
        frame_level_feature = self.frame_transformer(frame_level_feature)
...
class CoarsePyramid(nn.Module):
    ...
    def forward(self, feat_dict, ssl=False):
        ...
        short_feat = 0.5 * (self.short_conv[0](frame_level_feat) + frame_level_feat)
        middle_feat = 0.5 * (self.middle_conv[0](frame_level_feat) + frame_level_feat)
        long_feat = 0.5 * (self.long_conv[0](middle_feat) + middle_feat)

        device = torch.device('cuda' if torch.cuda.is_available() else "cpu")
        temporal_fuse = FeatureFusionModule().to(device)
        short_feat = short_feat.to(device)
        middle_feat = middle_feat.to(device)
        long_feat = long_feat.to(device)
        frame_level_feat = temporal_fuse(short_feat, middle_feat, long_feat)
        frame_level_feat = frame_level_feat.permute(0, 2, 1)
...
class FeatureFusionModule(nn.Module):
    ...
    def forward(self, short_term_feature, mid_term_feature, long_term_feature):
        combined_feature = torch.cat((short_term_feature, mid_term_feature, long_term_feature), dim=-1)
        output_feature = self.linear(combined_feature)
